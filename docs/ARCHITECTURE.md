...
- `UNLISTED_ORG_SLUGS` hides orgs from the front-page picker and the sitemap
  (`src/util/orgVisibility.ts`, applied in `listPublicOrgs`). It is a
  **discovery** filter, not access control: `/org/<slug>` still serves an
  unlisted org, by design, so test orgs stay testable. Never treat an
  unlisted slug as secret.
